  ...
  return (
    <div className="deck-rise flex flex-col items-center px-4 py-16 text-center sm:py-24">
      ...
      <h2 className="mt-5 max-w-xl text-2xl font-semibold tracking-tight sm:text-3xl">
        Read the shape of anyone&rsquo;s GitHub
      </h2>
      ...
      <div className="mt-7 flex flex-wrap items-center justify-center gap-2">
        ...
        {SUGGESTIONS.map((name) => (
          <button
            ...
            type="button"
            onClick={() => onPick(name)}
            className="cursor-pointer rounded-full border border-line px-3 py-1 font-mono text-xs text-ink-2 transition-colors hover:border-accent hover:text-accent"
          >
            {name}
          </button>
        ))}
...
function SkeletonBlock({ className }: { className: string }) {
  return <div className={`deck-skeleton rounded-md ${className}`} />;
}
    ...
    <div aria-busy="true">
      ...
      <section className="card p-5 sm:p-6">
        <div className="flex flex-col gap-5 sm:flex-row">
          <SkeletonBlock className="size-20 shrink-0 rounded-2xl sm:size-22" />
        ...
        <SkeletonBlock className="mt-6 h-[10px] w-full rounded-[3px]" />
      </section>

      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: 6 }, (_, i) => (
          <SkeletonBlock key={i} className="h-[168px] rounded-[14px]" />
        ))}
      </div>
    </div>
  );
...
export function NoReposState({ login }: { login: string }) {
  return (
    <div className="card deck-rise flex flex-col items-center px-6 py-14 text-center">
      <DeckMark className="size-9 text-ink-3" />
      <h3 className="mt-4 text-lg font-semibold">No public repositories</h3>
...
export function NoMatchesState({ onClear }: { onClear: () => void }) {
  return (
    <div className="card deck-rise flex flex-col items-center px-6 py-14 text-center">
      ...
      <h3 className="mt-4 text-lg font-semibold">Nothing matches those filters</h3>
      <p className="mt-2 max-w-sm text-sm text-ink-2">
      ...
      <button
        type="button"
        onClick={onClear}
        className="mt-5 cursor-pointer rounded-lg border border-line px-3 py-1.5 text-sm text-ink-2 transition-colors hover:border-accent hover:text-accent"
      >
        Clear filters
      </button>
    </div>
...
export function ErrorState({
  ...
  return (
    <div
      role="alert"
      className="card deck-rise flex flex-col items-center px-6 py-14 text-center"
      style={{ borderColor: 'color-mix(in oklab, var(--critical) 35%, transparent)' }}
    >
      <AlertIcon className="size-9 text-critical" />
      <h3 className="mt-4 text-lg font-semibold">{copy.headline}</h3>
      <p className="mt-2 max-w-md text-sm leading-relaxed text-ink-2">{copy.body}</p>
      {copy.retryable && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 cursor-pointer rounded-lg bg-accent px-4 py-1.5 text-sm font-medium text-accent-ink transition-opacity hover:opacity-90"
        >
          Try again
        </button>
      )}
